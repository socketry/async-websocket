		...
		while message = connection.read
			...
			if event and event[:text] =~ /^\/(.*?)$/
				begin
					...
					if result.is_a? Hash
						Protocol::WebSocket::TextMessage.generate(result).send(connection)
					else
						Protocol::WebSocket::TextMessage.generate({result: result}).send(connection)
					end
				rescue => error
					Protocol::WebSocket::TextMessage.generate({error: error}).send(connection)
				end
			else
